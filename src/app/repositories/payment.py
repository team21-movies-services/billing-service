...
class PaymentRepositoryABC(ABC):
    ...
    @abstractmethod
    async def update_payment(self, payment_id: str, status_id: UUID) -> UserPaymentResponse | None:
        raise NotImplementedError
...
class PaymentRepository(SQLAlchemyRepo, PaymentRepositoryABC):
    ...
    async def update_payment(self, payment_id: str, status_id: UUID) -> UserPaymentResponse | None:
        stmt = update(UserPayment).values(payment_id=payment_id, pay_status_id=status_id).returning(UserPayment)
        result = await self._session.execute(stmt)
        if user_payment := result.scalar():
            return user_payment
        return None
